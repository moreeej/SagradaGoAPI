...
async function createWedding(req, res) {
  try {
    ...
    // Handle admin-created bookings
    let user = null;
    let userName = '';
    let userEmail = '';
    
    if (uid === 'admin') {
      // For admin-created bookings, use the provided email and full_name
      userEmail = req.body.email || '';
      userName = req.body.full_name || '';
    } else {
      // For regular user bookings, look up the user
      user = await UserModel.findOne({ uid, is_deleted: false });
    ...
    if (!uploadedDocuments.groom_1x1 && !req.body.groom_1x1) {
      return res.status(400).json({ message: "Groom 1x1 photo is required." });
    }
    if (!uploadedDocuments.bride_1x1 && !req.body.bride_1x1) {
    ...
    // Send booking received email to user (skip if created by admin)
    if (uid !== 'admin' && userEmail) {
      try {
        const emailHtml = EmailService.generateBookingReceivedEmail(userName, "Wedding", {
          transaction_id: weddingData.transaction_id,
          date: weddingData.date,
          time: weddingData.time,
        });
        await EmailService.sendEmail(userEmail, "Wedding Booking Received - Sagrada Familia Parish", emailHtml, null, userName);
