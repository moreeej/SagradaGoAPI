...
async function createAnointing(req, res) {
  try {
    ...
    try {
      ...
      if (userEmail) {
        const userName = `${user.first_name} ${user.middle_name || ''} ${user.last_name}`.trim();
        const emailHtml = EmailService.generateBookingReceivedEmail(userName, "Anointing of the Sick", {
          transaction_id,
          date,
          time: normalizeTime(time),
        });
        await EmailService.sendEmail(userEmail, "Anointing of the Sick Booking Received - Sagrada Familia Parish", emailHtml, null, userName);
